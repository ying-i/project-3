...
def index():
    ...
    # ** is used for dictionary unpacking 取出字典的值. 
...
# won't be used
@pages.route("/draft", methods=["GET", "POST"])
def index_draft():
    if request.method == "POST": #receiving form data
        image = request.files["image"] #image_file is a binary file
        ...
        # reformatting the "formatted_date", why we need the date in two different formats?
        #because "formatted_date" is a string,
        # datetime.datetime.strptime(story[2]), "%Y-%m-%d": make python understand it as date not a string
        stories_with_date = [
            (
                story[0], story[1], story[2], datetime.datetime.strptime(story[2], "%Y-%m-%d").strftime("%b %d")
            )
            for story in stories # we want to do this for every story in the storiies list
        ]
      
        return render_template("index.html", stories = stories_with_date)
